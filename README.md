    ...
    6. Reference other resource
       ...
       ```json
       {
         "authmycognitoresourceUserPoolId": {
           // The format out here is `<category><resource-name><attribute-name>` - we have defined all of these in the `backend-config.json` file above
           "Fn::GetAtt": [
...
8.  Step Functions
    ...
    - input / output
      ...
      - You can create custom input by using Parameters field
        ```json
            "Parameters": {
